...
@Autonomous
public class AutoBlue_Premiere extends LinearOpMode {
    ...
    void one() {
        moveY(autoConstant.warehouse_position, -5);
        suck(0);
    }

    void two() {
        moveY(autoConstant.warehouse_position + 20, -5);
        suck(0);
    }
    ...
    void five() {
        moveY(autoConstant.warehouse_position + 25, -5);
        timelapse = robot.runtime.milliseconds();
        while (opModeIsActive() && robot.distance.getDistance(DistanceUnit.CM) > 15 && robot.runtime.milliseconds() - timelapse < 1750) {
            robot.Odometry();
    ...
    public void stretch() {
        ...
        robot.setRail(rail, 1);
        sleep(100);
        turn(autoConstant.turn_degree - 7);
    }
    ...
    void back() {
        ...
        while (opModeIsActive() && tube.getDistance(DistanceUnit.CM) > 6) {
            robot.Odometry();
            robot.move(-0.1, -1, (getHeading() + 7) * 0.025);
        }
        robot.move(0, 0, 0);
        robot.setY(105);
        //back to put
        while (opModeIsActive() && robot.getY() > autoConstant.start_position + 5) {
            robot.Odometry();
            robot.move(-0.05, Range.clip(-1 * (robot.getY() - autoConstant.start_position - 20) / 45, -1, -0.18), (getHeading() + 7) * 0.07);
            if (robot.getY() < autoConstant.start_position + 30) robot.setBasket(true);
