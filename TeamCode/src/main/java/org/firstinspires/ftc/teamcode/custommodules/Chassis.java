...
public class Chassis extends Thread {
    ...
    public static double max_accelerate = 40,
            terminate_power = 0.04,
            code_per_inch_right = 63.43,
            code_per_inch_forward = 56.5,
            turn_weight = 12.04,
            k_turn = 0.99948978616416859481694720973317,
            trace_kp = 0.2;
    ...
    public Chassis(LinearOpMode opMode) {
        this.opMode = opMode;
        lf = new Wheel(opMode.hardwareMap.dcMotor.get("lf"), DcMotor.Direction.FORWARD);
        lb = new Wheel(opMode.hardwareMap.dcMotor.get("lb"), DcMotor.Direction.FORWARD);
        rf = new Wheel(opMode.hardwareMap.dcMotor.get("rf"), DcMotor.Direction.REVERSE);
        rb = new Wheel(opMode.hardwareMap.dcMotor.get("rb"), DcMotor.Direction.REVERSE);
    ...
    public void move_to(double target_X, double target_Y, double target_Direction, double maxSpeed) {
        ...
        while (opMode.opModeIsActive() && (opMode.getRuntime() - startTime) < totalTime + 1) {
            if ((opMode.getRuntime() - startTime) <= totalTime)
                incomplete_Rate = (Math.cos(Math.PI * (opMode.getRuntime() - startTime) / totalTime) + 1) / 2;
            ...
            move(error_X, error_Y, error_direction, Math.sqrt(speed) * trace_kp);
        }
        lf.setPower(0);
        lb.setPower(0);
        rf.setPower(0);
    ...
    public void refreshPosition() {
        ...
        opMode.telemetry.addData("d_turn", d_turn);
